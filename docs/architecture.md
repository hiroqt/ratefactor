...
- **Health/uptime**: `GET /api/health` and `GET /api/monitoring/uptime` call `trackUptimeHeartbeat()`, which calls `checkSystemHealth()` (`src/lib/uptime.ts`). `POST /api/monitoring/uptime` does **not** call `checkSystemHealth()` — it only records a `downtime_incident` event or a custom heartbeat ping (PostHog/Sentry) from the request body and never runs the dependency check itself. `checkSystemHealth()` reports on four "dependencies": PostgreSQL (checked only by inspecting `DATABASE_URL`'s shape, not a live query), **Supabase** (checked only by presence of `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`), Better Auth, and the showcase engine (always reports healthy). The Supabase check is stale monitoring debt from before the Neon migration — no code path in the application imports a Supabase client for request handling. See [deployment-operations.md](./deployment-operations.md#monitoring) for how to interpret this.
