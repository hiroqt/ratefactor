...
**Known implementation debt — a route that skips this mapping**: the comment-report route (`POST /api/portfolios/{id}/comments/{commentId}/report`) inserts the raw `authUser.id` directly into `comment_reports.reporter_id` (a `uuid` FK to `profiles.id`) without calling `resolveCanonicalProfileId` first. For a non-UUID Better Auth id this can fail the insert; the failure is caught and only logged, not surfaced to the caller. Full detail: [portfolio-system.md](./portfolio-system.md#reports).
