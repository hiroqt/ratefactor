...
| Method | Route | Auth | Purpose | Persistence / side effects |
|---|---|---|---|---|
| GET | `/api/portfolios` | Optional | Public/personalized portfolio feed: filter by category/domain/host/query, sort, paginate. | Read-only. Always resolves the caller's profile id/likes first if a session is present (small SQL lookups). Default query (`category=All`, no `domain`/host/`q`, `sort=highest_rated`, `offset=0`, `limit=20`) still serves the feed itself from the 45s L1 cache when fresh, authenticated or not; all other queries — including any `?domain=` — hit Postgres directly, parameterized via `p.domains @> ARRAY[$n]` (GIN-indexed). `category` and `domain` are independent params and compose. Each portfolio includes `domains: PortfolioDomain[]` (`[]` for rows predating this column). See [portfolio-system.md](./portfolio-system.md#reading-public-feed). |
| POST | `/api/portfolios` | Required | Submit a new portfolio. | Inserts `portfolios` (+`profiles` if needed); runs GitHub verification; invalidates portfolio + developer caches. Rate-limited (`SUBMIT_PORTFOLIO`, 5/24h). Requires `domains`: 1–5 values from the `PORTFOLIO_DOMAINS` allowlist, no duplicates — rejected (400) otherwise, independent of `category`/`techStack`. See [portfolio-system.md](./portfolio-system.md#creation). |
