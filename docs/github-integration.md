...
- **README**: REST contents API first (works for the default branch); on failure, tries raw file fetch from `main` then `master` branches directly (`raw.githubusercontent.com`). All README markdown is passed through `sanitizeReadmeMarkdown` (`src/lib/github/readme.ts`), which strips `<script>`/`<iframe>`/`<object>`/`<embed>`/`<meta>`/`<base>`/`<link>`/`<form>` tags, inline event handlers (`on*=`), and `javascript:`/`vbscript:` URLs before it's ever rendered.
