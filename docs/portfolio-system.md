...
- **Known operational debt — inconsistent missing-config handling**: `POST /api/uploads/portfolio` (signature issuance) calls `getCloudinaryConfig()` inside a `try/catch` and returns a clean `503 { detail: "Portfolio image uploads are unavailable." }` if Cloudinary env vars are missing. `DELETE /api/uploads/portfolio` calls `getCloudinaryConfig()` **outside** any `try/catch`, before the receipt is even checked — if Cloudinary is unconfigured, this throws and surfaces as an unhandled `500`, not a documented `502`/`503` response. The upload path degrades gracefully; the cleanup path does not.
...
- Query params: `category` (enum or `"All"`), `host` (domain/TLD filter, `.tld` prefix or substring match), `sort` (`highest_rated` default, `most_liked`, `most_discussed`, `latest`, `showcase`), `q` (free-text), `limit` (1–50, default 20), `offset` (default 0).
- **Default-query fast path**: the handler always resolves the caller's profile id (if a session is present) and, if resolved, the caller's `likes` rows, *before* checking the cache — this is two small SQL lookups regardless of whether the feed itself ends up cached. When the query is exactly `{category: All, host: (none), sort: highest_rated, offset: 0, limit: 20}` and the in-memory cache (`src/lib/dynamic-portfolios.ts`) is fresh (<45s old), the feed payload itself is served entirely from memory — zero portfolio/comment queries — with the caller's `isLiked` flags merged into the cached rows per-request. For a fully anonymous default request, this means zero SQL of any kind is run once the cache is warm. A matching `If-None-Match` returns `304` with an empty body (anonymous only — the 304 short-circuit requires no resolved `currentProfileId`).
- **All other queries** go to Postgres. Filtering is built as a parameterized `WHERE` clause (`p.status = 'published'` always, plus optional category/host/text filters); the free-text search matches title, tagline, description, author name/username, category, hostname, portfolio/demo URL, and tech stack, all via case-insensitive `LIKE` with a manually escaped wildcard (`isAllowedPortfolioThumbnail`-style `%`/`_`/`\` escaping) — **not** the `idx_portfolios_search` GIN/tsvector index (that index exists but full-text query syntax isn't used here). Sorting maps to one of four `ORDER BY` clauses matching the sort param. Pagination is `LIMIT`/`OFFSET` in SQL.
...
- Upserts into `comment_reports` (`ON CONFLICT (comment_id, reporter_id) DO UPDATE`) — one report per reporter per comment, re-reportable with updated details.
...
- Does **not** call `invalidatePortfoliosCache()` after the insert, unlike every other comment/like/rating mutation in this file.

### Known implementation debt

- **Reporter identity is not resolved to a canonical profile id.** `comment_reports.reporter_id` is a `uuid` FK to `profiles.id`, but the route (`src/app/api/portfolios/[id]/comments/[commentId]/report/route.ts`) inserts `authUser.id` — the raw Better Auth session id — directly as `reporter_id`, without first calling `resolveCanonicalProfileId` (see [auth-and-identity.md](./auth-and-identity.md#profile-identity-mapping)). When `authUser.id` is not itself a UUID (an email/password-flow-generated Better Auth id), the insert violates the `uuid`-typed column/FK. The `INSERT` is wrapped in a `try/catch` that only `console.warn`s on failure (`"[Comment Report] DB insert fallback"`) — the route still returns its normal `200` "flagged for review" response to the caller either way, so a persistence failure here is silent to both the reporter and any caller-visible state. Net effect: for a non-UUID Better Auth id, the report can silently fail to persist even though the API reports success. This is documented as-is, not fixed, per the scope of this pass.
- **No cache invalidation after a comment report.** A report can flip `comments.status` to `'flagged'` (removing it from the "approved and not reported" set the feed/comments queries filter on) once the 3rd report lands, but the report route never calls `invalidatePortfoliosCache()`. The portfolio L1 process-local cache (`src/lib/dynamic-portfolios.ts`, 45s TTL — see [architecture.md](./architecture.md#caching-and-egress)) can keep serving a previously cached default-feed response whose embedded `comments`/`commentsCount` still include a comment that has since been flagged/hidden, until the cache's own TTL naturally expires. Same risk applies to the non-default-query comment batch load, which reads live from Postgres and is therefore not affected — only the 45s L1-cached default-feed path is stale-risk here.
